...
def get_files():
  ...
  if is_aac:
    ...
    with current_app.config['db'] as (conn, cur):
      cur.execute("SELECT program, academic_year FROM report WHERE has_been_reviewed=FALSE AND creator_id=%(creator_id)s", {"creator_id": asker})
      conn.commit()
      to_be_reviewed = cur.fetchall()
      cur.execute("SELECT program, academic_year FROM report WHERE has_been_reviewed=TRUE AND creator_id=%(creator_id)s", {"creator_id": asker})
      conn.commit()
      complete = cur.fetchall()
